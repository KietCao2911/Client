...
export const fetchGetKhoHang = createAsyncThunk("KhoHang/fetchGetKhoHang",async(params)=>
{
    const {maSP} = params;
    const res = await KhoHangAPI.fetchGetKhoHang(maSP)
    return res;
})
export const fetchGetProducts = createAsyncThunk("KhoHang/fetchGetProducts",async(params)=>{
    const {maChiNhanh,query} = params;
    console.log({maChiNhanh,query});
