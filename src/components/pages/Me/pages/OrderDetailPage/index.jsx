...
import OrderPageSkeleton from "../OrderPage/Components/Skeleton"
const OrderDetailPage =()=>
{
    const {id} = useParams();
    const dispatch = useDispatch()
    const {order,loading} = useSelector(state=>state.Me)
    console.log({order})
    useEffect(()=>
    {
        dispatch(MeAPI.getMyOrder({id}))
    },[id])
    return loading?<OrderPageSkeleton></OrderPageSkeleton>: <Row gutter={[20,20]}>
    <Col span={24}>
