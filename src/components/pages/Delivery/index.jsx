...
const DeliveryPage = () => {
  ...
  const { ghnAPI, chiTietNhapXuats, totalPrice, totalQty, phiShip, finalPrice } = useSelector(state => state.GioHang);
  ...
  const handleOrder = (paymentMethod) => {
    if (Object.keys(user).length !== 0) {
      if (user.info && user.info.length > 0) {
        ...
        if(!error)
        {
          dispatch(ThanhToanApi.fetchPostWithGuess({
            DiaChi: params, hoaDonDetails: chiTietNhapXuats,
      ...
      {
        dispatch(ThanhToanApi.fetchPostWithGuess({
          DiaChi: GuessInfo, hoaDonDetails: chiTietNhapXuats,
